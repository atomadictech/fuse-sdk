...
S2S is the flagship **single-repo** product story: one half-baked repository in, one install-and-run-ready polyglot product out.

### Public SDK vs local engine

The **hosted SDK** (`FuseClient`, this repository) is the HTTP/MCP contract for catalog, compile, classify, verification, and store maintenance. It does **not** replace the full on-disk S2S orchestrator.

The **local Fuse engine** (`atomadic-fuse-next` during development; promoted to `atomadic-fuse` when stable) exposes:

- CLI: `fuse s2s` (alias `spaghetti-to-shippable`)
- MCP: `spaghetti_to_shippable` / `s2s` on the `fuse-engine` stdio server

### Dry-run vs live

| Mode | Behavior | Trust |
|------|----------|-------|
| **Dry-run** (default) | Stage plan, constraints check, no destructive writes | Public — no trust token |
| **Live** (`--live`) | Sync source, heal, emit, fitness gate, T5 bow, optional polyglot | Trust-gated: `gate_intent` → malicious gate → Nexus or `ATOMADIC_TRUST_TOKEN` |

Public S2S enforces **one source repo**, blocks cross-repo emergent evolution, and caps engine tier at **4** with a **T5 product bow**.

### Fitness gate

Before the T5 bow, live S2S runs `process_gate_fitness_stateful` on the lead emitted CNAE. Dry-run defers fitness execution but still reports the planned lead atom.

Further reading:

- [S2S and Engine Boundary](S2S_AND_ENGINE_BOUNDARY.md)
- [Omega S2S pipeline](../../docs/SPAGHETTI_TO_SHIPPABLE.md)
